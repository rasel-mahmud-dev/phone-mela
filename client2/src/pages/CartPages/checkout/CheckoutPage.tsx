    ...
    (async function (){
      if(auth.id){
        ...
        for (const responseElement of response.data) {
          if(responseElement.is_default === 1){
            setSelectShippingAddress(responseElement.id)
          }
        }
      ...
      <div className="">
        ...
        <div className="mt-8">
         ...
         <div className="flex justify-between items-center">
          ...
          <div className="flex">
            <Input2 value={shippingAddress.first_name} name="first_name" onChange={handleChange} className="mr-2"  type="text" placeholder="First Name (optional)"/>
            <Input2 value={shippingAddress.last_name} name="last_name" onChange={handleChange} className="ml-2"  type="text" placeholder="Last Name"/>
          </div>
    ...
    return (
      <div className="mt-4">
        ...
        { recentShippingAddress && recentShippingAddress.map((recentShipAddr: ShippingAddress, i)=>(
          <div onClick={()=>recentShipAddr.id && handleChangeShippingAddress(recentShipAddr.id)}
               className="flex items-center my-2 py-2 cursor-pointer">
            <input
              type="radio"
              name="shipping-address"
              // value={selectShippingAddress === recentShipAddr.id}
              checked={recentShipAddr.id === selectShippingAddress}
            />
            
            <div className={["border hover:border-primary-400/40 hover:bg-primary-10 rounded mx-2 p-2 text-sm", selectShippingAddress === recentShipAddr.id ? "border-primary-400/40 bg-primary-10" : ""].join(" ")}>
  ...
  function handleToPay() {
      if(selectShippingAddress !== 0){
        if(recentShippingAddress) {
          let s = recentShippingAddress.find(r => r.id === selectShippingAddress)
            ...
            <div className="cart_items">
              ...
              <div className="flex justify-between sm:flex-row flex-col">
                ...
                <div className="sm:flex-5 lg:flex-2">
                  <OrderSummary
                    cartProducts={cartProducts}
                    shippingAddress={selectShippingAddress !== 0 ? recentShippingAddress ? recentShippingAddress.find(r=>r.id === selectShippingAddress) : false: false}
                    nextLevel={
                      <div className="mt-5">
                        <button
