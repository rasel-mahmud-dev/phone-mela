...
export const filterAbleList: ProductAttributesType[] = [
  ...
  },
  {
    ...
    options: [
      ...
      {name: "7000mAh & Above", value: [7000, 0]}
    ],
    optionsForAdd: [
...
const FilterSidebar:FC<FilterSidebarProps> = ({brands}) => {
  ...
  const selectedAttributeFilter = useSelector((state: RootStateType)=> state.productState.selectedAttributeFilter)
  ...
  const [expandSection, setExpandSection] = React.useState<string[]>([])
  ...
  function removeSelectedItem(section_attributeName, val){
    const updatedSelectedFilter = {...selectedAttributeFilter}
    let i = updatedSelectedFilter[section_attributeName].findIndex(att=>att.name === val.name)
    if(i !== -1){
      // already selected than remove it
    ...
    if(data && Object.keys(data).length > 0){
      return (
        Object.keys(data).map(item=>(
          <div className="flex flex-wrap" key={item}>
            { data[item] && data[item].map((val, ind)=>
              <span key={ind} className="select-item flex items-center text-xs bg-primary-400/30 px-1 py-0.5 m-0.5 rounded">
                <span>{val.name}</span>
                <FontAwesomeIcon onClick={()=>removeSelectedItem(item, val)} className="ml-2 select-del" icon={faTimes} />
              </span>
            )}
          </div>
        ))
  ...
  return (
    <div>
      <div ref={selectedAttributeRef} className="flex flex-wrap px-4 mb-4 mt-1">
