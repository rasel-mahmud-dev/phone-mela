...
export const fetchBrands = (cb:(data: BrandType[])=>void)=> async(dispatch: (arg0: FetchBrands) => void) => {
  try{
...
export function toggleHandleCart(product: AddCartPayload, isShowPopup: boolean = false){
    ...
    if (auth.isAuthenticated) {
      ...
      if (index === -1) {
        ...
        if (product) {
          ...
          try{
            const r = await api.post("api/add-cart", newCart)
            if (r.status === 201) {
              newCart.id = r.data.id
...
export function toggleHandleWishlist(product: AddWishlistPayload, isShowPopup: boolean = false, cb?: any){
  return async function (dispatch: (args: TOGGLE_WISHLIST_ACTION | SET_ACTION_TYPE)=> TOGGLE_WISHLIST_ACTION, getState: any){
    ...
    if(auth && auth.id) {
      ...
      if(index === -1){
        ...
        const r =  await api.post("api/remove-wishlist", {wishlist_id: wishItem.id, customer_id: auth.id})
        if(r.status === 201){
...
export function fetchCart(customer_id: number){
